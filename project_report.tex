...
% Additional colors used in TikZ diagrams
...
\begin{figure}[H]
\centering
% Increased node distance to avoid overlapping nodes on tight layouts
\begin{tikzpicture}[node distance=3.2cm, scale=0.9, transform shape]
...
\subsection{Features}
This component supports three primary operation modes which are useful for
testing, demonstration, and batch usage:
\begin{itemize}[leftmargin=*]
    \item \textbf{Interactive Mode:} Default behavior with user prompts; enter commands like \texttt{print N}, \texttt{stats}, and \texttt{exit}.
    \item \textbf{Direct Print Mode (\texttt{-p N}):} Non-interactive mode that waits until \texttt{N} values are consumed then prints them and exits; useful for scripting and batch tests.
    \item \textbf{Demo Mode (\texttt{-d}):} Runs self-contained demos of the sequencer, event counter, and ring buffer primitives to validate behavior.
    \item \textbf{Statistics:} Track consumption metrics (produced/consumed/in-flight) via the \texttt{stats} command.
    \item \textbf{Graceful Shutdown:} Close ring buffer and wake waiting threads to enable clean exit.
\end{itemize}
